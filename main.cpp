...
void spawnApple()
{
    ...
    // Find valid position that isn't on snake, not on walls)
    bool validPosition = false;
    int attempts = 0;

    while (!validPosition && attempts < 100)
    {
        // Random position within bounds keeping away from the edges
...
{
    if (snake.empty() || gameState != PLAYING)
        return;
...
void initApples()//places the apples
{
    apples.clear();
    spawnApple();
}
...
//Wall Functions 
void initWalls()//places the walls
{
    walls.clear();
    // Boundary walls
...
void drawTexturedWall(float x, float z, float w, float d, float h)//this one draws the walls
{
...
void drawTexturedCube(float size)//this draws the cubes of the snake body with the it's texture
{
    float s = size / 2.0f;
    glEnable(GL_TEXTURE_2D);
...
void drawSnakeHead(Direction facing)
{
    ...
    case DOWN:
        ...
        break; // Looking downconst [state, setState] = useState(initialValue); (+Z)
    case LEFT:
        glRotatef(90.0f, 0.0f, 1.0f, 0.0f);
        break; // Looking left (-X)
    case RIGHT:
        glRotatef(-90.0f, 0.0f, 1.0f, 0.0f);
        break; // Looking right (+X)
    }
